...
export default function HeroSection() {
  const { setQuery, fetch } = useFoodStore();
  const [term, setTerm] = useState("");
  return (
    <section className="w-full bg-gradient-to-r from-foodSecondary to-foodPrimary">
      <div className="mx-auto max-w-6xl px-4 sm:px-6 py-10 sm:py-16 md:py-24 grid grid-cols-1 md:grid-cols-2 items-center gap-6 md:gap-10">
        <div className="text-white">
          ...
          <div className="mt-6 sm:mt-8 bg-white rounded-xl shadow-lg p-3 sm:p-4">
            ...
            <div className="w-full mt-3 sm:mt-4 flex items-center gap-2 sm:gap-3">
              <div className="flex-1 flex items-center gap-2 bg-zinc-100 rounded-full px-3 sm:px-4 py-3 text-zinc-600">
                ...
                <input
                  ...
                  placeholder="What food do you like to eat today?"
                  className="flex-1 bg-transparent outline-none placeholder:text-zinc-400"
                />
              </div>
              <FoodButton
                text="Find Meal"
                styles="rounded-sm px-4 py-3 text-sm sm:text-base"
                onClick={() => { setQuery(term); fetch(term); }}
              />
            </div>
          </div>
        </div>
